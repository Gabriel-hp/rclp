...
@section('content')
    ...
    <div class="container mt-4">
        ...
        <form method="GET" action="{{ route('dashboard') }}" class="mb-4 mt-5">
            <div class="row g-2">
                <div class="col-md-3">
                    <input type="text" name="numero_chamado" class="form-control" placeholder="Número do Protocolo"
                        value="{{ request('numero_chamado') }}">
                </div>
                <div class="col-md-2">
                    <select name="status" class="form-control">
                        ...
                        <option value="Em atendimento" {{ request('status') == 'Em atendimento' ? 'selected' : '' }}>Em
                            atendimento</option>
                        <option value="Aguardando" {{ request('status') == 'Aguardando' ? 'selected' : '' }}>Aguardando
                        </option>
                        <option value="Data Center" {{ request('status') == 'Data Center' ? 'selected' : '' }}>Data Center
                        </option>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-1">
                    <select name="nivel" class="form-control">
                        ...
                        <option value="Data Center" {{ request('nivel') == 'Data Center' ? 'selected' : '' }}>Data Center
                        </option>
                ...
                <div class="col-md-2">
                    <select name="ordenacao" class="form-control">
                        <option value="recente" {{ request('ordenacao') == 'recente' ? 'selected' : '' }}>Mais Recentes
                        </option>
                    ...
                    <tbody id="tabelaChamados">
                        @foreach ($chamadosCollection as $chamado)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge bg-{{ $chamado->status === 'Em atendimento' ? 'success' : ($chamado->status === 'Aguardando' ? 'info' : 'warning') }}">
                                        {{ $chamado->status }}
                                    </span>
                                </td>
                                <td>{{ $chamado->nivel }}</td>
                                ...
                                <td>
                                    <a href="https://logicpro.movidesk.com/Ticket/EditByProtocol/{{ $chamado->protocolo }}"
                                        target="_blank">
        ...
        <script>
            ...
            function atualizarChamados() {
                fetch('{{ route("dashboard.update") }}')

                    .then(response => response.json())
                    .then(data => {
                        let tabela = document.getElementById('tabelaChamados');
                        let linhasExistentes = new Set(); // Armazena os protocolos já inseridos

                        data.forEach(ticket => {
                            let protocolo = ticket.protocolo;
                            linhasExistentes.add(protocolo);

                            // Verifica se já existe a linha para o protocolo
                            let linhaExistente = document.querySelector(`tr[data-protocolo="${protocolo}"]`);

                            if (!linhaExistente) {
                                let newRow = tabela.insertRow();
                                newRow.setAttribute('data-protocolo', protocolo); // Adiciona um identificador à linha
                                ...
                                let cells = [
                                    ...
                                    ticket.nivel,
                                    ticket.aberto_em,
                                    ticket.escalonamento,
                                    ticket.lastUpdate,
                                    `<a href="https://logicpro.movidesk.com/Ticket/EditByProtocol/${protocolo}" target="_blank">
                                <button type="button" class="btn btn-light">Acesse o ticket</button>
                            </a>`
                                ];
                        ...
                        // Remover linhas que não estão mais nos dados recebidos
                        document.querySelectorAll("#tabelaChamados tr").forEach(row => {
                            let protocolo = row.getAttribute("data-protocolo");
                            if (!linhasExistentes.has(protocolo)) {
                                row.remove();
                            }
                        });
            ...
            // Atualiza os chamados a cada 30 segundos
            setInterval(atualizarChamados, 30);
